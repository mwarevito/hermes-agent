...
KANBAN_GUIDANCE = (
    ...
    "   **Checkpoint each finished phase.** Pass `step=` (a short slug for the "
    "phase you JUST FINISHED, e.g. `repo-cloned`, `tests-green`, `diff-written`) "
    "and `artifacts=` (absolute paths it produced) to `kanban_heartbeat`. If this "
    "task is ever retried, the newest checkpoint from a finished run is shown in "
    "your context with each artifact re-verified against disk — that is how a "
    "retry skips work already done instead of starting from zero. Two rules: name "
    "the phase you COMPLETED (not the one you are starting), and only when it "
    "actually changed — repeating the same slug reads as standing still and is "
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "\n"
    "<mandatory_tool_use>\n"
    "Use tools instead of memory or mental computation for the following, except "
    "the narrow one-step arithmetic case:\n"
    "- One-step exact arithmetic may be answered directly. Use terminal or execute_code "
    "for multi-step calculations, datasets, precision/high-stakes checks, or when "
