...
| `HERMES_STREAM_HARD_TIMEOUT_SECONDS` | Absolute ceiling on ONE streaming provider call, measured from the start of the call (default: `1500`, `0` disables). Unlike every other stream watchdog it is not reset by incoming frames, so a stream that drips content-free keepalives forever is still bounded. Sits above every stale floor so it never clamps a healthy long generation; when it fires it logs the reason and elapsed time at ERROR and surfaces a user-visible abort rather than a silent retry. |
| `HERMES_API_CALL_TOTAL_BUDGET_SECONDS` | Wall-clock budget for ONE *logical* API call, spanning all of its retries and fallbacks (default: `3600`, `0` disables). Every other timeout is per-attempt and the retry loop multiplies it. Gates the START of another attempt only — an attempt already in flight is never interrupted, so a healthy slow generation cannot be truncated by it. |
